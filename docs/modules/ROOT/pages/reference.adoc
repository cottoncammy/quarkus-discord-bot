...
To listen to Gateway events, simply declare a method with an event type parameter and annotate it with `@GatewayEvent`. The method can either be imperative, returning `void` or reactive, returning `Mono`, `Uni`, `Flux`, or `Multi`.
...
And here's the same method written reactively:
...
class DeletePersistentMessage {
    void onMessageDelete(@GatewayEvent MessageDeleteEvent event, EntityManager em) {
